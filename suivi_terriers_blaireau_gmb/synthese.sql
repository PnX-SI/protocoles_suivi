...
--
--  - choisir les valeurs de champs de nomenclatures qui seront propres au modules-- ce fichier contient une variable :module_code (ou :'module_code')
-- utiliser psql avec l'option -v module_code=<module_code>

-- ne pas remplacer cette variable, elle est indispensable pour les scripts d'installations
-- le module pouvant être installé avec un code différent de l'original--suiviblaireau/*cor_deter_obsvation as (	
...
DROP VIEW IF EXISTS gn_monitoring.v_synthese_suivi_terriers_blaireau_gmb;
CREATE VIEW gn_monitoring.v_synthese_suivi_terriers_blaireau_gmb AS
...
SELECT 
		...
		end)::int  AS id_nomenclature_obj_count, --objet du denombrement
		(case 
		...
		end)::int  AS id_nomenclature_type_count, --objet du denombrement
	...
	JOIN observers obs ON obs.id_base_visit = v.id_base_visit 
 	--left JOIN determinateurs_monit on determinateurs_monit.id_observation = o.id_observation
