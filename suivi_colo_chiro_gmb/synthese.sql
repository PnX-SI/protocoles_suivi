...
DROP VIEW IF EXISTS gn_monitoring.v_synthese_suivi_colo_chiro_gmb;
CREATE VIEW gn_monitoring.v_synthese_suivi_colo_chiro_gmb AS
...
SELECT
		...
		(case 
			when obscompl.data ->> 'statut_biologique'::text = 'Estivation' then '34' --estivation
			when obscompl.data ->> 'statut_biologique'::text = 'Hibernation' then '33'--hibernation
			when obscompl.data ->> 'statut_biologique'::text = 'Reproduction (mise-bas et l''élevage des jeunes)' then '32' --reproduction
			when obscompl.data ->> 'statut_biologique'::text = 'Swarming (accouplements)' then '32' --reprduction
			when obscompl.data ->> 'statut_biologique'::text = 'Inconnu ou Incertain' then '29' --inconnu
		end)::int AS id_nomenclature_bio_status, -- STATUT_BIO
		(case
		...
		(case 
			when obscompl.data -> 'methode_observation'::text = 'null' then '58' --inconnu
		...
		(case 
			when obscompl.data -> 'nombre_individus' = 'null' then '1'
			else obscompl.data -> 'nombre_individus'
