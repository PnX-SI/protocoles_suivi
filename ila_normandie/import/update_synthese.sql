DROP VIEW gn_monitoring.v_hors_synthese_ila;

CREATE OR REPLACE VIEW gn_monitoring.v_hors_synthese_ila
 AS
 WITH source AS (
         SELECT sc.id_source,
            mo.id_module
          ...
          WHERE sc.name_source::text = 'MONITORING_ILA'::text
        ), ds AS (
 ...
 SELECT o.uuid_observation AS unique_id_sinp,
    ...
    s.altitude_min,
    s.altitude_max,
    ((((((s.data ->> 'commune'::text) || ' - '::text) || s.sites_group_code::text) || ' - '::text) || s.sites_group_name::text) || ' - '::text) || s.base_site_code::text AS place_name,
    s.the_geom_4326,
    s.the_geom_point,
    ...
    v.id_module,
    (((((((((((('passage : '::text || (v.data ->> 'num_passage'::text)) || ' | habitat : '::text) || (v.data ->> 'habitat'::text)) || ' | gestion : '::text) || COALESCE(v.data ->> 'gestion'::text, '/'::text)) || ' | impact : '::text) || COALESCE(v.data ->> 'impact'::text, '/'::text)) || ' | vent : '::text) || ref_nomenclatures.get_nomenclature_label((v.data ->> 'id_nomenclature_vt'::text)::integer)::text) || ' | couverture nuageuse : '::text) || ref_nomenclatures.get_nomenclature_label((v.data ->> 'id_nomenclature_cn'::text)::integer)::text) || ' | temperature : '::text) || ref_nomenclatures.get_nomenclature_label((v.data ->> 'id_nomenclature_tp'::text)::integer)::text AS comment_context,
        CASE
