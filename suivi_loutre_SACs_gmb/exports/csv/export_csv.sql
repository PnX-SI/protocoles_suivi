...
data_monitoring AS (
	...
	SELECT
		...
		case 
			when obscompl.data ->> 'nb_epreinte_w'::text = 'null' then null
			else obscompl.data ->> 'nb_epreinte_w'::text
		end AS nb_epreinte_w,
		case
...
(ref_nomenclatures.get_nomenclature_label(datamonit.cadre_prosp::int, 'fr'))::text as cadre_prosp, 
