...
CREATE VIEW gn_monitoring.v_synthese_flore_biotopoe AS
WITH source AS (
	SELECT id_source
    FROM gn_synthese.t_sources
	WHERE name_source = CONCAT('MONITORING_', UPPER('flore_biotopoe'))
	LIMIT 1
), observers AS (
    SELECT
 ...
 SELECT o.uuid_observation AS unique_id_sinp,
  ...
  WHERE m.module_code::text = 'flore_biotopoe'::text;
