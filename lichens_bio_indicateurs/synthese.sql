CREATE OR REPLACE VIEW gn_monitoring.v_synthese_lichens_bio_indicateurs AS
WITH source AS (
    SELECT
        id_source
 ...
 SELECT to2.uuid_observation AS unique_id_sinp,
    ...
    v.id_base_site,
    v.id_base_visit,
    (toc."data"->>'id_nomenclature_observation_status')::integer as id_nomenclature_observation_status,
    1 AS count_min,
    1 AS count_max
...
LEFT JOIN LATERAL ( SELECT string_agg(concat(UPPER(tr.nom_role), ' ', tr.prenom_role), ', ') AS observers, array_agg(tr.id_role) as ids_observers
