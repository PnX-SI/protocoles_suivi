...
create table perso.tmp_bkp_monitoring_pop as (SELECT o.uuid_observation,
                                                     tsg.id_sites_group,
                                              ...
                                              FROM gn_monitoring.t_base_visits v
                                                       JOIN gn_monitoring.t_visit_complements vc
                                                            on v.id_base_visit = vc.id_base_visit
                                                       ...
                                                       JOIN gn_monitoring.t_sites_groups tsg
                                                            ON sc.id_sites_group = tsg.id_sites_group
                                                       ...
                                                       LEFT JOIN gn_commons.t_medias tm ON (tm.id_table_location =
                                                                                            gn_commons.get_table_location_id('gn_monitoring', 't_observations') AND
                                                                                            tm.uuid_attached_row =
                                                                                            o.uuid_observation)
                                                       JOIN taxonomie.taxref t ON t.cd_nom = o.cd_nom
                                              WHERE m.module_code ilike 'popreptile');
...
/*todo : attention, script à personnaliser pour restreindre aux données à mettre à jour (pop reptile de l'ancien module uniquement*/
...
WITH selection AS
         (SELECT oc.id_observation,
                 t.lb_nom,
                 oc.DATA,
                 oc.DATA ->> 'presence'                 AS presence,
                 CASE
                     WHEN oc.DATA ->> 'nombre_compte' IS NOT NULL
                         THEN ref_nomenclatures.get_id_nomenclature('TYP_DENBR', 'Co')
                     WHEN oc.DATA ->> 'nombre_estime_min' IS NOT NULL
                         THEN ref_nomenclatures.get_id_nomenclature('TYP_DENBR', 'Es')
                     ELSE NULL
...
FROM selection
WHERE nb_co IS NULL
  AND nb_es_min IS NULL
  AND nb_es_max IS NULL
ORDER BY id_observation;
...
WITH to_update AS
         (SELECT oc.id_observation,
          ...
          WHERE m.module_code ilike 'popreptile'
            AND oc."data" ->> 'count_min' IS NULL
            AND oc.DATA ->> 'presence' = 'Non'
            AND lb_nom = 'Squamata')
UPDATE gn_monitoring.t_observation_complements
SET "data" = jsonb_build_object('presence', 'Non') || jsonb_build_object('count_min', 0) ||
             jsonb_build_object('count_max', 0) ||
             jsonb_build_object('id_nomenclature_typ_denbr', NULL) ||
             jsonb_build_object('id_nomenclature_stade', NULL) || jsonb_build_object('id_nomenclature_sex', NULL)
WHERE t_observation_complements.id_observation IN (SELECT id_observation FROM to_update);

-- Gestion des données de présence

WITH selection AS
         (SELECT distinct oc.id_observation,
                          t.lb_nom,
                          oc.DATA,
                          oc.DATA ->> 'presence'                 AS presence,
                          CASE
          ...
          WHERE m.module_code ilike 'popreptile'
            AND oc."data" ->> 'count_min' IS NULL
            AND (oc.DATA ->> 'presence' = 'Oui' or oc.DATA ->> 'presence' is null)),
     to_update AS
         (SELECT id_observation,
                 ...
                 CASE
                     ...
                     WHEN stade_vie IN ('Juvéniles', 'Nouveaux-nés')
                         THEN ref_nomenclatures.get_id_nomenclature('STADE_VIE', '3')
                     ELSE NULL
                     END AS id_life_stage,
                 "data"
          FROM selection
          WHERE id_observation NOT IN
                (SELECT id_observation FROM selection WHERE nb_co IS NULL AND nb_es_min IS NULL AND nb_es_max IS NULL))
UPDATE gn_monitoring.t_observation_complements
SET "data" = jsonb_build_object('presence', u.presence) || jsonb_build_object('count_min', u.count_min) ||
             jsonb_build_object('count_max', u.count_max) ||
             jsonb_build_object('id_nomenclature_typ_denbr', u.id_nomenclature_typ_denbr) ||
             jsonb_build_object('id_nomenclature_stade', u.id_life_stage) ||
             jsonb_build_object('id_nomenclature_sex', NULL)
FROM to_update u
WHERE t_observation_complements.id_observation = u.id_observation;


-- gestion des données de présence pour lesquelles le champ presence est à null
...
WITH selection AS
         (SELECT distinct oc.*
          FROM gn_monitoring.t_observation_complements oc
                   ...
                   LEFT JOIN gn_commons.t_modules m USING (id_module)
          WHERE m.module_code ilike 'popreptile'
            AND (oc.data -> 'count_max')::int > 0
            and oc.data -> 'presence' = 'null')
UPDATE gn_monitoring.t_observation_complements obs
SET data = obs.data || '{"presence":"Oui"}'
FROM selection u
WHERE obs.id_observation = u.id_observation;
...
WITH selection AS
         (SELECT vc.*,
          ...
          WHERE m.module_code ilike 'popreptile'
            AND sc."data" ->> 'methode_prospection' IS NOT NULL
            AND vc."data" ->> 'methode_prospection' IS NULL),
     to_update AS
         (SELECT *,
                 CASE
                     WHEN method_p = 'Par plaques uniquement' THEN ARRAY ['Par plaques']
                     WHEN method_p = 'Par plaques et observation directe'
                         THEN ARRAY ['Par observation directe', 'Par plaques']
...
WITH selection AS
     ...
     to_update AS
         (SELECT id_base_site,
                 CASE
                     WHEN base_site_description IS NULL THEN concat('nb plaques: ', nb_plaques)
...
WITH to_update AS
         (SELECT tc.id_base_site,
                 tc.DATA - 'nb_plaques' - 'milieu_bordier' - 'type_materiaux' - 'milieu_homogene' -
                 'milieu_anthropique' - 'frequentation_humaine' - 'milieu_transect_autre' - 'microhabitat_favorable' -
                 'milieu_mosaique_vegetale' - 'methode_prospection'
                     AS new_json_data
          FROM gn_monitoring.t_site_complements tc
                   JOIN gn_commons.t_modules m ON m.id_module = tc.id_module
          WHERE m.module_code ilike 'popreptile')
...
WITH to_update AS
         (SELECT tg.id_sites_group, tg.DATA - 'expertise' AS new_json_data
          FROM gn_monitoring.t_sites_groups tg
...
WITH to_update AS
         (SELECT tc.id_base_site
          FROM gn_monitoring.t_site_complements tc
                   JOIN gn_commons.t_modules m ON m.id_module = tc.id_module
                   join gn_monitoring.t_base_sites using (id_base_site)
          WHERE m.module_code ilike 'popreptile'
            --lpo
            and (base_site_name not ilike '%import_shf%'))
