...
DROP VIEW IF EXISTS gn_monitoring.v_synthese_RHOMEOOdonates;
CREATE VIEW gn_monitoring.v_synthese_RHOMEOOdonates AS
...
WITH source AS (
	...
	WHERE name_source = CONCAT('MONITORING_', UPPER('RHOMEOOdonates'))
	LIMIT 1
...
SELECT
	...
	JOIN observers obs ON obs.id_base_visit = v.id_base_visit
 	...
 	LEFT JOIN LATERAL ref_geo.fct_get_altitude_intersection(s.the_geom_local) alt (altitude_min, altitude_max)
        ON TRUE
    WHERE m.module_code = 'RHOMEOOdonates'
    ;
