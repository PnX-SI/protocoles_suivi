...
AS WITH observers AS (
 ...
 SELECT s.id_base_site AS code_aire,
    s.base_site_name AS nom_aire,
...
-- Export des sites
CREATE OR REPLACE VIEW gn_monitoring.v_export_nidif_gypa_sites
AS SELECT s.id_base_site AS code_aire,
    s.base_site_name AS nom_aire,
    st_astext(s.geom) AS geom_wkt,
    a.jname ->> 'COM'::text AS commune,
    a.jname ->> 'SEC'::text AS secteur,
   ...
   FROM gn_monitoring.t_base_sites s
     ...
     JOIN gn_commons.t_modules mod ON mod.id_module = csm.id_module AND mod.module_code::text = 'nidif_gypa'::text
