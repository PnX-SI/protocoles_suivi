...
CREATE OR REPLACE VIEW gn_monitoring.v_synthese_nidif_gypa AS
WITH source AS (
	SELECT
        id_source
    FROM gn_synthese.t_sources
	WHERE name_source = 'MONITORING_NIDIF_GYPA'
	LIMIT 1
), observers AS (
    SELECT
    ...
    JOIN gn_monitoring.cor_visit_observer cvo ON cvo.id_base_visit  = v.id_base_visit
 ...
 SELECT to2.uuid_observation AS unique_id_sinp,
    ...
    alt.altitude_min,
    alt.altitude_max,
    st_centroid(s.geom) AS the_geom_4326, -- on prend le centroid pour avoir une meilleur représentation dans la synthese et ne pas dupliquer des lignes
