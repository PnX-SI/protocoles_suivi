-- TODO
CREATE OR REPLACE VIEW gn_monitoring.v_synthese_ecrevisses_pattes_blanches AS
WITH source AS (
    SELECT
        id_source
    FROM gn_synthese.t_sources
    WHERE name_source = CONCAT('MONITORING_', UPPER('ECREVISSES_PATTES_BLANCHES'))
    LIMIT 1
)
 SELECT to2.uuid_observation AS unique_id_sinp,
  ...
  WHERE m.module_code::text = 'ecrevisses_pattes_blanches'::TEXT AND tn.cd_nomenclature  = 'Pr';


ALTER VIEW gn_monitoring.v_synthese_ecrevisses_pattes_blanches  OWNER TO geonatadmin;