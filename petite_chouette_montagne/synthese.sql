...
DROP VIEW IF EXISTS gn_monitoring.v_synthese_petite_chouette_montagne;
CREATE VIEW gn_monitoring.v_synthese_petite_chouette_montagne AS
...
SELECT
		...
		ref_nomenclatures.get_id_nomenclature('METH_OBS', '1') id_nomenclature_obs_technique, -- METH_OBS
		...
		(oc."data"::json#>>'{chev_sexe}')::int AS id_nomenclature_sex, -- SEXE
		...
		CASE WHEN (oc."data"::json#>>'{nb_total}')::int > 0
		...
		(oc."data"::json#>>'{nb_total}')::int AS count_min,
		(oc."data"::json#>>'{nb_total}')::int AS count_max,
		o.id_observation,
		o.cd_nom,
		...
		v.id_base_site,
		v.id_base_visit,
		jsonb_set(v.data, array['device_repasse'],to_jsonb(ref_nomenclatures.get_nomenclature_label((v."data"::json#>>'{device_repasse}')::int))) || oc.data - '{nb_total,chev_sexe}'::text[] additional_data
